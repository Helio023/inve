...
const SmartToolbox = ({
  ...
  const NestedIndicator = () => (
    <div className="mb-4 animate-in slide-in-from-top-2 duration-300">
      <div className="bg-blue-600 p-3 rounded-2xl flex items-center justify-between shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-3">
            {variants.map((preset: any) => (
              ...
              >
                ...
                <div className="px-1 font-bold text-[9px] uppercase tracking-tighter text-slate-500 group-hover:text-blue-600 transition-colors">
                  {preset.label}
                </div>
              </button>
            ))}
              ...
              {Object.entries(BLOCK_DEFINITIONS)
                ...
                .map(([key, def]: [string, any]) => (
                  <button
                    ...
                    className={cn(
                      "flex flex-col items-center justify-center p-3 rounded-[20px] border transition-all duration-300 active:scale-95 shadow-sm h-24 bg-white group relative overflow-hidden",
                      colorMap[def.color],
                    )}
                  >
...
export function EditorClient({
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-100 overflow-hidden relative font-sans antialiased">
      <EditorHeader
        ...
        onUpdateSettings={async (s) =>
          await updateEventSettingsAction(eventId, s)
        }
      />
      ...
      <div className="flex-1 flex overflow-hidden relative">
          ...
          <aside className="w-72 bg-white border-r flex flex-col z-10 shadow-sm overflow-hidden">
            ...
            <div className="p-4 flex-1 overflow-y-auto no-scrollbar">
              <SmartToolbox
                ...
                isNested={!!addingTo}
                onCancelNested={() => setAddingTo(null)}
              />
            </div>
          </aside>
        )}
        ...
        <main className="flex-1 relative overflow-hidden bg-slate-100 flex flex-col items-center justify-between">
          {/* NAVEGAÇÃO LATERAL DESKTOP (POSICIONAMENTO SEGURO) */}
          {!isPreview && isDesktop && (
            <div className="absolute inset-x-0 top-[50%] -translate-y-1/2 flex justify-between px-6 pointer-events-none z-[40]">
              <Button
                variant="secondary"
                size="icon"
                className={cn(
                  "h-14 w-14 rounded-full shadow-2xl pointer-events-auto bg-white/90 backdrop-blur hover:bg-white transition-all",
                  activePageIndex === 0 && "opacity-0 invisible",
                )}
              ...
              <Button
                ...
                className={cn(
                  "h-14 w-14 rounded-full shadow-2xl pointer-events-auto bg-white/90 backdrop-blur hover:bg-white transition-all",
                  activePageIndex === totalPages - 1 && "opacity-0 invisible",
                )}
                onClick={() => setActivePageId(pages[activePageIndex + 1].id)}
              >
          ...
          <div className="flex-1 w-full flex items-center justify-center p-4 min-h-0 overflow-y-auto no-scrollbar">
            <div className="py-12 flex flex-col items-center w-full">
              <EditorCanvas
                ...
                onAddChild={(parentId: string, colIndex: number) => {
                  ...
                  if (!isDesktop) setToolsOpen(true);
                }}
          ...
          {!isPreview && isDesktop && (
            <div className="w-full bg-white border-t p-3 flex items-center justify-center gap-4 z-50 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                ...
                <div className="flex items-center gap-1 max-w-[400px] overflow-x-auto no-scrollbar scroll-smooth px-2 py-1">
                  {pages.map((p: any, idx: number) => (
                    <button
                      ...
                      className={cn(
                        "min-w-[32px] h-8 rounded-lg text-[10px] font-black transition-all border flex items-center justify-center shrink-0",
                        p.id === activePageId
              ...
              <Button
                size="sm"
                variant="outline"
                className="h-9 gap-2 text-[10px] font-bold border-dashed border-slate-300 hover:border-blue-300 hover:text-blue-600 transition-all"
                onClick={addPage}
              >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="end"
                  className="w-52 rounded-xl shadow-xl border-slate-100"
                >
                  ...
                  <DropdownMenuItem onClick={() => duplicatePage(activePageId)}>
                    <Copy className="w-4 h-4 mr-2" /> Duplicar Slide
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => reorderPage("LEFT")}
                    disabled={activePageIndex === 0}
                  >
                    <ArrowLeft className="w-4 h-4 mr-2" /> Mover p/ Trás
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => reorderPage("RIGHT")}
                    disabled={activePageIndex === totalPages - 1}
                  >
                    <ArrowRight className="w-4 h-4 mr-2" /> Mover p/ Frente
                  </DropdownMenuItem>
                  ...
                  >
                    <Trash2 className="w-4 h-4 mr-2" /> Apagar Slide
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-3 flex items-center gap-2 z-50 pb-[calc(env(safe-area-inset-bottom)+12px)] shadow-2xl">
          ...
          >
            <SheetTrigger asChild>
              ...
              >
                {!!addingTo ? "Escolher p/ Coluna" : "Adicionar"}
              </Button>
            </SheetTrigger>
            <SheetContent
              side="bottom"
            ...
            >
              <SheetHeader className="px-6 py-4 bg-white border-b text-left">
                <SheetTitle className="text-lg font-bold">
                  {!!addingTo ? "Inserir na Coluna" : "Biblioteca"}
                </SheetTitle>
                <SheetDescription className="sr-only">
                  Adicione um novo bloco
                </SheetDescription>
              </SheetHeader>
              <div className="p-6 pb-20 overflow-y-auto max-h-[75dvh] no-scrollbar">
                <SmartToolbox
                  onAdd={handleAddBlock}
                  isNested={!!addingTo}
                  onCancelNested={() => {
          ...
          <Button
            variant="outline"
            size="icon"
            className="h-12 w-12 rounded-xl bg-slate-50 border-slate-200"
            onClick={() => {
              selectBlock(null);
              setActiveTab("page");
              setSettingsOpen(true);
          ...
          <Sheet open={isSettingsOpen} onOpenChange={setSettingsOpen}>
            <SheetContent
              side="bottom"
              className="rounded-t-[2.5rem] p-0 max-h-[88vh] bg-white border-none focus-visible:ring-0"
            >
              <SheetHeader className="sr-only">
                <SheetTitle>Propriedades</SheetTitle>
                <SheetDescription>Configurações</SheetDescription>
              </SheetHeader>
              <div className="p-6 overflow-y-auto max-h-[85vh] pb-32 no-scrollbar">
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  <TabsList className="grid grid-cols-2 mb-6 h-10 bg-slate-100 p-1">
                    <TabsTrigger
                      value="block"
                    ...
                    <TabsTrigger value="page" className="text-[10px] font-bold">
                      PÁGINA
                    </TabsTrigger>
                  </TabsList>
      ...
      {/* PAGINAÇÃO MOBILE FLUTUANTE COM TUDO RESTAURADO */}
      {!isPreview && !isDesktop && (
        <div className="md:hidden fixed bottom-24 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-white/95 backdrop-blur-md shadow-2xl border border-slate-200 rounded-full px-3 py-1.5 animate-in slide-in-from-bottom-4">
          <Button
            variant="ghost"
            size="icon"
          ...
          <div className="flex items-center gap-1.5 max-w-[120px] overflow-x-auto no-scrollbar scroll-smooth px-1">
            {pages.map((p: any, idx: number) => (
              <button
                key={p.id}
                id={`page-btn-mobile-${p.id}`}
                onClick={() => setActivePageId(p.id)}
                className={cn(
                  "min-w-[28px] h-7 rounded-lg text-[10px] font-black transition-all flex items-center justify-center shrink-0 border",
                  p.id === activePageId
            ...
            <button
              onClick={addPage}
              className="min-w-[28px] h-7 rounded-lg bg-blue-50 text-blue-600 border border-blue-100 flex items-center justify-center shrink-0"
            >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="center"
              side="top"
              className="w-56 mb-2 rounded-2xl shadow-2xl border-slate-100 bg-white"
            >
              <DropdownMenuLabel className="text-[10px] font-bold uppercase text-slate-400 px-3 py-2">
                Gestão de Slides
              </DropdownMenuLabel>
              ...
              >
                <Copy className="w-4 h-4 mr-2 opacity-70" /> Duplicar Página
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => reorderPage("LEFT")}
                disabled={activePageIndex === 0}
                className="rounded-xl py-2.5 cursor-pointer"
              >
                <ArrowLeft className="w-4 h-4 mr-2 opacity-70" /> Mover p/ Trás
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => reorderPage("RIGHT")}
                disabled={activePageIndex === totalPages - 1}
                className="rounded-xl py-2.5 cursor-pointer"
              >
                <ArrowRight className="w-4 h-4 mr-2 opacity-70" /> Mover p/
                Frente
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem
                onClick={() => deletePage(activePageId)}
                className="text-red-600 focus:bg-red-50 focus:text-red-700 py-2.5 cursor-pointer"
                disabled={pages.length === 1}
              >
                <Trash2 className="w-4 h-4 mr-2" /> Apagar Página
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
