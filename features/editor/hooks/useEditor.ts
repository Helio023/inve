...
//     items: [{ time: "09:00", title: "Início", description: "Recepção dos convidados" }],
//   },
//   CAROUSEL: {
//     images: [{ url: "", alt: "" }],
//     autoplay: true,
...
//   SONG_REQUEST: {
//     title: "Que música não pode faltar?",
...
//   DRESS_CODE: {
//     title: "Dress Code",
//     description: "Traje Formal / Gala",
//     image: "",
//   },
//   FAQ: {
//     items: [{ q: "Existe estacionamento?", a: "Sim, gratuito no local." }],
//   },
// };
...
const MAX_HISTORY = 6; // Estado atual + 5 níveis de desfazer
...
const INITIAL_BLOCK_DATA: Record<BlockType, any> = {
  ...
  IMAGE: { url: "", alt: "" },
  VIDEO: { url: "" },
  MAP: { venueName: "Local", address: "Morada aqui", link: "", buttonText: "Abrir GPS" },
  COUNTDOWN: { date: new Date().toISOString(), label: "Faltam apenas" },
  RSVP: { title: "Confirme sua Presença", description: "Sua presença é importante", buttonText: "Confirmar" },
  COLUMNS: { cols: 2, children: { col0: [], col1: [] } },
  MENU: {
    isInteractive: false,
    sections: [{ title: "Entradas", items: [{ name: "Prato", description: "" }] }],
  },
  SCHEDULE: {
    title: "Cronograma do Evento",
  ...
  BUTTON: { text: "Clique Aqui", url: "https://qonvip.com" },
  SONG_REQUEST: { title: "Sugira uma música", placeholder: "Nome da música...", buttonText: "Pedir" },
  ICON: { name: "Heart", size: 32, repeat: 1 },
  DIVIDER: { align: "center", width: "50%" },
  SAVE_THE_DATE: { title: "Nosso Casamento", date: "2026-05-20", dateDisplay: "20 de Maio", buttonText: "Salvar Data" },
...
export function useEditor(initialData: IPage[] = []) {
  const [pages, _setPages] = useState<IPage[]>(
    initialData.length > 0 ? initialData : [{
      id: uuidv4(), title: "Capa", order: 0, blocks: [], styles: { backgroundColor: "#ffffff" },
    }]
  );

  const [history, setHistory] = useState<IPage[][]>([initialData.length > 0 ? initialData : pages]);
  const [pointer, setPointer] = useState(0);
  const [activePageId, setActivePageId] = useState<string>(pages[0]?.id);
  const [selectedBlockId, setSelectedBlockId] = useState<string | null>(null);

  // Wrapper seguro para setPages com gestão de histórico
  const setPages = useCallback((newPagesOrFn: IPage[] | ((prev: IPage[]) => IPage[])) => {
    _setPages((current) => {
      const next = typeof newPagesOrFn === "function" ? newPagesOrFn(current) : newPagesOrFn;
  ...
  const addPage = useCallback(() => {
    const newPage: IPage = {
      id: uuidv4(), title: `Pág ${pages.length + 1}`, order: pages.length, blocks: [],
  ...
  const reorderPage = useCallback((dir: "LEFT" | "RIGHT") => {
    setPages(prev => {
  ...
  const addBlock = useCallback((type: BlockType, pId?: string, cIdx?: number, content?: any, styles?: any) => {
    const b: IBlock = { 
      id: uuidv4(), 
      type, 
    ...
    setPages(prev => prev.map(p => {
      ...
      return { ...p, blocks: p.blocks.map(bl => {
        if (bl.id === pId && bl.type === "COLUMNS") {
          const k = `col${cIdx}`;
          const children = bl.content.children || {};
          return { ...bl, content: { ...bl.content, children: { ...children, [k]: [...(children[k] || []), b] } } };
        }
        return bl;
      })};
    }));
  ...
  const updateBlock = useCallback((id: string, content: any) => 
    setPages(prev => prev.map(p => ({ ...p, blocks: updateRecursive(p.blocks, id, content, "content") }))), [setPages]);

  const updateStyles = useCallback((id: string, styles: any) => 
    setPages(prev => prev.map(p => ({ ...p, blocks: updateRecursive(p.blocks, id, styles, "styles") }))), [setPages]);

  const removeBlock = useCallback((id: string) => {
    if (selectedBlockId === id) setSelectedBlockId(null);
    setPages(prev => prev.map(p => ({ ...p, blocks: filterRecursive(p.blocks, id) })));
  ...
  const moveBlock = useCallback((id: string, dir: "UP" | "DOWN") => {
    const move = (arr: IBlock[]): IBlock[] => {
      ...
      if (idx !== -1) {
        ...
        if (target >= 0 && target < res.length) [res[idx], res[target]] = [res[target], res[idx]];
        return res;
      }
      return arr.map(b => b.type === "COLUMNS" ? { ...b, content: { ...b.content, children: Object.fromEntries(Object.entries(b.content.children).map(([k, v]) => [k, move(v as IBlock[])])) } } : b);
    };
  ...
  const duplicateBlock = useCallback((id: string) => {
    const dup = (arr: IBlock[]): IBlock[] => {
      ...
      return arr.map(b => b.type === "COLUMNS" ? { ...b, content: { ...b.content, children: Object.fromEntries(Object.entries(b.content.children).map(([k, v]) => [k, dup(v as IBlock[])])) } } : b);
    };
    setPages(prev => prev.map(p => ({ ...p, blocks: dup(p.blocks) })));
  }, [setPages]);

  const copyBlockToPage = useCallback((id: string, tId: string) => {
    setPages(prev => {
      let blockToCopy: IBlock | undefined;
      for (const p of prev) { 
      ...
      if (!blockToCopy) return prev;
      return prev.map(p => p.id === tId ? { ...p, blocks: [...p.blocks, cloneBlockWithNewIds(blockToCopy!)] } : p);
    });
  }, [setPages]);
  ...
  return {
    ...
    activePageId,
    selectedBlockId,
    selectedBlock: findBlockRecursive(pages.find(p => p.id === activePageId)?.blocks || [], selectedBlockId || ""),
    ...
    duplicatePage,
    movePage,
    updatePageTitle: (id: string, title: string) => setPages(prev => prev.map(p => p.id === id ? {...p, title} : p)),
    updatePageStyles: (id: string, styles: any) => setPages(prev => prev.map(p => p.id === id ? {...p, styles: {...p.styles, ...styles}} : p)),
    reorderPage,
