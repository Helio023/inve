...
export function useEditor(initialData: IPage[] = []) {
  ...
  const setPages = useCallback(
    (newPagesOrFn: IPage[] | ((prev: IPage[]) => IPage[])) => {
      _setPages((current) => {
        ...
        setHistory((prev) => {
          // Ao realizar nova ação, descartamos o futuro (Forward History) para evitar inconsistências
          const newHistory = prev.slice(0, pointer + 1);
          const updatedHistory = [...newHistory, next];
          
          if (updatedHistory.length > MAX_HISTORY) {
            const shifted = updatedHistory.slice(1);
            setPointer(shifted.length - 1);
            return shifted;
          }
  ...
  const redo = useCallback(() => {
    if (pointer < history.length - 1) {
      ...
      const nextPages = history[nextIndex];
      _setPages(nextPages);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey || e.metaKey) {
        if (e.key === "z") {
          e.preventDefault();
          if (e.shiftKey) {
            if (redo()) toast.info("Ação refeita", { duration: 1000 });
          } else {
  ...
  const deletePage = useCallback(
    (id: string) => {
      setPages((prev) => {
        ...
        const indexToDelete = prev.findIndex((p) => p.id === id);
        const filtered = prev.filter((p) => p.id !== id);

        if (activePageId === id) {
          // Seleciona a página anterior, ou a primeira se a anterior não existir
          const nextIndex = indexToDelete > 0 ? indexToDelete - 1 : 0;
          setActivePageId(filtered[nextIndex].id);
        }
        return filtered;
      });
    },
