...
//     position: "relative",
//     zIndex: isSelected ? 20 : 1, // Traz para a frente ao editar
//   };
...
//               }}
//               disabled={isFirst}
//               className="px-2 h-full hover:bg-white/20 rounded-l-md border-r border-white/10"
//               title="Mover Cima"
//             >
...
//       {/* CONTEÚDO REAL DO BLOCO (Com Overflow para cortar bordas arredondadas) */}
...
//         {/* DIVIDER: Este não usa os estilos globais de altura para não esticar */}
//         {block.type === "DIVIDER" && (
//           <div
//             style={{
//               width: "100%",
...
//         {/* Blocos Complexos (O pointer-events-none impede interação interna durante edição) */}
//         {block.type === "RSVP" && (
//           <div
//             className={cn(
...
import {
  ...
  MoreHorizontal,
  ArrowRightCircle,
  Plus, // Importante para o botão nas colunas
} from "lucide-react";
import { DEFAULT_STYLES } from "../types";
import {
  DropdownMenu,
...
export const BlockRenderer = ({
  ...
  pages,
  onCopyToPage,
  isFirst, // Recebe a prop para saber se é o primeiro bloco
  onAddChild, // Recebe a função para adicionar filhos (colunas)
}: any) => {
  const s = { ...DEFAULT_STYLES, ...block.styles };
  ...
  const contentStyles: React.CSSProperties = {
    ...
    // Alturas
    height: block.type === "DIVIDER" ? "auto" : s.height || "auto",
    minHeight: !block.content?.image && block.type === "HERO" ? "auto" : (block.type === "DIVIDER" ? "auto" : s.height),
    ...
    boxShadow: s.shadow === "none" ? "none" : 
               s.shadow === "sm" ? "0 1px 2px rgba(0,0,0,0.1)" : 
    ...
    // Layout Flex (Correção para Alinhamento)
    display: (block.type === "DIVIDER" || block.type === "MAP") ? "block" : "flex",
    flexDirection: "column",
    justifyContent: "center",
    alignItems: "stretch", // IMPORTANTE: Força os filhos a ocuparem a largura toda
  };
  ...
  return (
    ...
    >
      {/* ======================================================== */}
      {/* UI DE SELEÇÃO (RETÂNGULO PERFEITO & TOOLBAR INTELIGENTE) */}
      {/* ======================================================== */}
      {!isPreview && isSelected && (
        <>
          {/* 1. Borda de Seleção (Outline Retangular) */}
          <div className="absolute inset-0 border-2 border-blue-600 pointer-events-none z-50" />
          ...
          <div
            className={cn(
              "absolute flex bg-slate-900 text-white rounded-md shadow-xl z-[60] items-center h-8 border border-white/20 transition-all",
              // Se for o 1º bloco -> Toolbar DENTRO (top-2 right-2)
              // Se forem outros -> Toolbar FORA EM CIMA (-top-9 right-0)
              isFirst ? "top-2 right-2" : "-top-9 right-0",
            )}
          >
            <button
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 z-[70]">
                <DropdownMenuLabel className="text-[10px] uppercase text-slate-400">Ações</DropdownMenuLabel>
                <DropdownMenuItem onClick={() => onDuplicate(block.id)} className="cursor-pointer">
                  <Copy className="w-3 h-3 mr-2" /> Duplicar aqui
                </DropdownMenuItem>
                {pages && pages.length > 1 && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuLabel className="text-[10px] uppercase text-slate-400">Copiar para...</DropdownMenuLabel>
                    {pages.map((p: any, idx: number) => (
                      <DropdownMenuItem
                        key={p.id}
                        onClick={() => onCopyToPage(block.id, p.id)}
      ...
      <div style={contentStyles} className="overflow-hidden relative">
            ...
            <div className="relative w-full flex-1 overflow-hidden bg-slate-100/50 rounded-sm">
              {block.content.link ? (
                <iframe
                  ...
                  loading="lazy"
                  style={{ border: 0, display: "block", pointerEvents: "none" }}
                  src={block.content.link.includes("embed") ? block.content.link : ""}
                />
              ) : (
                <div className="flex flex-col items-center justify-center h-full text-slate-400">
                  <MapPin className="w-8 h-8 opacity-50 mb-2" />
        ...
        {block.type === "DIVIDER" && (
          ...
          >
            <div
              style={{
                width: block.content?.orientation === "vertical" ? "0px" : s.width || "100%",
                height: block.content?.orientation === "vertical" ? s.height || "100px" : "0px",
                borderTopWidth: block.content?.orientation !== "vertical" ? `${s.borderWidth || 2}px` : 0,
                borderLeftWidth: block.content?.orientation === "vertical" ? `${s.borderWidth || 2}px` : 0,
                borderStyle: (s.borderStyle as any) || "solid",
                borderColor: s.borderColor || "#000000",
                backgroundColor: "transparent",
              }}
        ...
        {/* Blocos Complexos */}
        {block.type === "RSVP" && (
          <div className={cn(isPreview ? "pointer-events-auto" : "pointer-events-none")}>
            <RsvpBlock content={block.content} styles={s} isEditorPreview={!isPreview} />
          </div>
        )}
        {block.type === "COUNTDOWN" && (
          <CountdownRenderer date={block.content?.date} styles={s} />
        ...
        {block.type === "MENU" && (
          <MenuRenderer content={block.content} styles={s} isPreview={isPreview} />
        )}
        {block.type === "SCHEDULE" && (
          <ScheduleRenderer content={block.content} styles={s} />
        )}
