  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
        ...
        {styles.backgroundImage && (
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-3">
            ...
            <Slider
              ...
              step={0.1}
              onValueChange={([val]) =>
                handleStyleUpdate({ backgroundOpacity: val })
              }
      ...
      <div className="space-y-4">
        <Label className="text-xs font-black uppercase text-slate-900 tracking-wider flex items-center gap-2">
          <Maximize className="w-4 h-4 text-emerald-500" /> Espaçamento da
          Página
        </Label>
        ...
        <SpacingControl
          ...
          onChange={(newVals) => {
            ...
            if (newVals.left !== undefined)
              updateObj.paddingLeft = newVals.left;
            handleStyleUpdate(updateObj);
          }}
        />
        <p className="text-[10px] text-slate-400 italic">
          Dica: Use padding lateral (Esq/Dir) para evitar que o conteúdo cole
          nas bordas em telemóveis.
        </p>
      </div>
    </div>
  );
