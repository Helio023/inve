...
export function SaveTheDateRenderer({ 
  ...
  // Lógica de Data para o Calendário (Formato YYYYMMDD)
  const rawDate = content?.date ? content.date.replace(/-/g, '') : "20260520";
  
  // Link do Google Calendar (Otimizado)
  const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(eventTitle)}&dates=${rawDate}T100000Z/${rawDate}T220000Z&details=Convite enviado via Qonvip.`;

  return (
    <div 
      className="w-full flex flex-col items-center text-center py-10 px-6 gap-8 pointer-events-auto"
      style={getContainerStyle(s)}
    >
      <div 
        className="h-[1px] w-16 opacity-20" 
      ...
      <div className="w-full flex justify-center items-center">
        <a 
          ...
          style={{
            ...btnStyle,
            backgroundColor: (s as any).btnBackgroundColor || "#0f172a",
            color: (s as any).btnColor || "#fff",
            borderRadius: (s as any).btnBorderRadius || 99,
            padding: `${(s as any).btnPaddingTop || 12}px ${(s as any).btnPaddingRight || 32}px ${(s as any).btnPaddingBottom || 12}px ${(s as any).btnPaddingLeft || 32}px`,
            display: "inline-flex",
