...
export function RsvpBlock({
  ...
  const inputStyle = {
    ...
    color: s.inputTextColor,
    borderColor: s.inputBorderColor,
    borderRadius: `${s.inputBorderRadius || 8}px`, // Garante leitura correta
    boxShadow: getShadow(s.inputShadow),
    fontSize: `${s.fontSize}px`,
    
    // --- CORREÇÃO: Lê a largura e o estilo específicos dos inputs ---
  ...
  const btnStyle = {
    ...
    // --- CORREÇÃO: Adicionado suporte para bordas no botão ---
  ...
  return (
    <div className="w-full" style={{ color: s.color, fontFamily: s.fontFamily, textAlign: s.textAlign as any }}>
      <div className="flex flex-col gap-6">
        <h2 className="font-serif font-bold" style={titleStyle}>
          {content.title || "Confirme sua Presença"}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <input type="radio" name="attendance" className="hidden" onClick={() => setStatus("CONFIRMED")}/>
              <CheckCircle2 className={`w-8 h-8 ${status === "CONFIRMED" ? "text-green-600" : "text-slate-300"}`}/>
              <span className={`font-bold ${status === "CONFIRMED" ? "text-green-700" : "text-slate-500"}`} style={{ fontSize: `${s.fontSize * 0.9}px` }}>
                Sim, vou
              </span>
            </label>
            ...
            >
              <input type="radio" name="attendance" className="hidden" onClick={() => setStatus("DECLINED")}/>
              <XCircle className={`w-8 h-8 ${status === "DECLINED" ? "text-red-600" : "text-slate-300"}`}/>
              <span className={`font-bold ${status === "DECLINED" ? "text-red-700" : "text-slate-500"}`} style={{ fontSize: `${s.fontSize * 0.9}px` }}>
                Não poderei
              </span>
            </label>
          </div>
          ...
          <Button type="submit" className="w-full h-12 font-bold transition-transform active:scale-95" disabled={loading || status === "PENDING"} style={btnStyle}>
            {loading ? <Loader2 className="animate-spin mr-2" /> : <Send className="mr-2 w-5 h-5" />}
            Enviar Resposta
          </Button>
        </form>
